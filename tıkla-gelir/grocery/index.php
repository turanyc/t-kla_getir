...
// Manav bilgisi (vendor_types.slug = 'grocery')
$stmt = $pdo->prepare("
    SELECT b.*, vt.name as vendor_type_name
    FROM businesses b
    JOIN vendor_types vt ON vt.id = b.vendor_type_id
    WHERE vt.slug = 'grocery' AND b.is_approved = 1
    LIMIT 1
");
$stmt->execute();
$grocery = $stmt->fetch();
if (!$grocery) {
    die("<div style='text-align:center;padding:50px;'><h2>Manav şu anda hizmet dışı.</h2></div>");
}
...
$groceryCategories = $pdo->prepare("
    ...
    ORDER BY mc.name
");
$groceryCategories->execute([$grocery_id]);
$groceryCategories = $groceryCategories->fetchAll();
...
<head>
    ...
    <title><?= htmlspecialchars($grocery['name']) ?> - Tıkla Gelir</title>
...
<div class="container">
    ...
    <header class="main-header">
        <div class="logo-section">
            <a href="../index.php" style="text-decoration: none; color: inherit;"><h1><i class="fas fa-carrot"></i> Tıkla Gelir</h1></a>
            <p><strong><?= htmlspecialchars($grocery['name']) ?></strong> - Online Manav</p>
        </div>
    ...
    <section class="welcome-section">
        <h2>🥬 <?= htmlspecialchars($grocery['name']) ?></h2>
        <p>Taze sebzeler, meyveler ve daha fazlası!</p>
    </section>
    ...
    <div class="main-cat-bar">
        ...
        ?>
            <a href="<?= $link ?>" class="main-cat-btn <?= $mc['slug']==='grocery' ? 'active' : '' ?>">
        ...
        <div class="products-grid">
            <?php foreach ($items as $item): ?>
                <div class="product-card">
                    ...
                    <button class="add-to-cart" onclick="addToCart(<?= $item['id'] ?>, '<?= htmlspecialchars($item['name']) ?>', <?= $item['price'] ?>, <?= $grocery_id ?>)"><i class="fas fa-cart-plus"></i> Sepete Ekle</button>
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
