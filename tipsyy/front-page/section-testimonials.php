<div class="front_page_section front_page_section_testimonials<?php
	$tipsy_scheme = tipsy_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $tipsy_scheme ) && ! tipsy_is_inherit( $tipsy_scheme ) ) {
		echo ' scheme_' . esc_attr( $tipsy_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( tipsy_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( tipsy_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$tipsy_css      = '';
		$tipsy_bg_image = tipsy_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$tipsy_anchor_icon = tipsy_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$tipsy_anchor_text = tipsy_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( tipsy_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$tipsy_css      = '';
			$tipsy_bg_mask  = tipsy_get_theme_option( 'front_page_testimonials_bg_mask' );
			$tipsy_bg_color_type = tipsy_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $tipsy_bg_color_type ) {
				$tipsy_bg_color = tipsy_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$tipsy_caption = tipsy_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $tipsy_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $tipsy_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $tipsy_caption, 'tipsy_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$tipsy_description = tipsy_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $tipsy_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $tipsy_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $tipsy_description ), 'tipsy_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! tipsy_exists_trx_addons() ) {
						...
						tipsy_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
