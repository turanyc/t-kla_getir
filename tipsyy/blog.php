...
if ( function_exists( 'tipsy_elementor_is_preview' ) && tipsy_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			tipsy_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	tipsy_new_main_query(
		array(
			'post_type'      => tipsy_get_theme_option( 'post_type' ),
			'category'       => tipsy_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'tipsy_filter_get_template_part', tipsy_blog_archive_get_template() ) );
}
