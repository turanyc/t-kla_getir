...
if ( apply_filters( 'tipsy_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
	...
	tipsy_show_post_featured_image( array(
		'thumb_bg'  => true,
		'popup'     => true,
        'class_avg' => in_array( $tipsy_post_format, array( 'video' ) )
            ? ( ! $video_autoplay
                ? 'content_wrap'
	...
	ob_end_clean();
	$tipsy_with_featured_image = tipsy_is_with_featured_image( $tipsy_post_header );
	...
	tipsy_show_post_title_and_meta( array(
										...
										'share_type'    => 'list',
										'show_labels'   => true,
										'author_avatar' => $tipsy_with_featured_image,
										'add_spaces'    => true,
										)
									);
	$tipsy_post_header .= ob_get_contents();
