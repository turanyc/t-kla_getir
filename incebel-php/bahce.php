...
$sorgu8=$baglanti->prepare("SELECT * FROM bahcedb");
$sorgu8->execute();
$sonuc8=$sorgu8->fetch();
?>
  <div class="keuneken-body">
    <div class="keuken-container">
      ...
      <div class="keuken-left-content">
        <h1 class="keuken-header-one"><?= $sonuc8["baslik"] ?></h1>
        <p class="keuken-description"><?= $sonuc8["paragraf"] ?>
        </p>

        <ul class="keuken-features">
          <li class="keuken-feature-item"><?= $sonuc8["madde1"] ?></li>
          <li class="keuken-feature-item"><?= $sonuc8["madde2"] ?></li>
          <li class="keuken-feature-item"><?= $sonuc8["madde3"] ?></li>
        </ul>
      ...
      <div class="keuken-right-content">
        <img src="img/<?= $sonuc8["foto"] ?>" alt="Fotoğraf">
      </div>
    </div>
  </div>
  <!-- Arama İkonu -->
  ...
  <div class="keuken-wrapper">
    ...
    <div class="keuken-left-section">
        <?php
        $sorgu9=$baglanti->prepare("SELECT * FROM bahcedb2");
        $sorgu9->execute();
        $sonuc9=$sorgu9->fetch();
        ?>
      <h2 class="keuken-header"><?= $sonuc9["baslik"] ?></h2>
      <p class="keuken-paragraph"><?= $sonuc9["paragraf"] ?></p>
      <img class="keuken-large-image" src="img/<?= $sonuc9["foto"] ?>" alt="Büyük Resim 1">

      <p class="keuken-paragraph"><?= $sonuc9["paragraf2"] ?></p>
