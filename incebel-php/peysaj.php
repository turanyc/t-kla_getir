...
$sorgu17=$baglanti->prepare("SELECT * FROM peysajdb");
$sorgu17->execute();
$sonuc17=$sorgu17->fetch();
?>
  <!-- Arama İkonu -->
  <div class="call-icon">
  ...
  <div class="keuneken-body">
    <div class="keuken-container">
      ...
      <div class="keuken-left-content">
        <h1 class="keuken-header-one"><?= $sonuc17["baslik"] ?></h1>
        <p class="keuken-description"><?= $sonuc17["paragraf"] ?>
        </p>

        <ul class="keuken-features">
          <li class="keuken-feature-item"><?= $sonuc17["madde1"] ?></li>
          <li class="keuken-feature-item"><?= $sonuc17["madde2"] ?></li>
          <li class="keuken-feature-item"><?= $sonuc17["madde3"] ?></li>
        </ul>
      ...
      <div class="keuken-right-content">
        <img src="<?= $sonuc17["foto"] ?>" alt="Fotoğraf">
      </div>
    </div>
  </div>
  <!--Keuken end-->
  ...
  <div class="keuken-wrapper">
    ...
    <div class="keuken-left-section">
        <?php
        $sorgu18=$baglanti->prepare("SELECT * FROM peysajdb2");
        $sorgu18->execute();
        $sonuc18=$sorgu18->fetch();
        ?>
      <h2 class="keuken-header"><?= $sonuc18["baslik"] ?></h2>
      <p class="keuken-paragraph"><?= $sonuc18["paragraf"] ?></p>
      <img class="keuken-large-image" src="<?= $sonuc17["foto"] ?>" alt="Büyük Resim 2">
      <p class="keuken-paragraph"><?= $sonuc18["paragraf2"] ?></p>
