...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
                        ...
                        <?php foreach ($required_plugins as $slug => $plugin) : ?>
                            <li data-slug="<?php echo esc_attr($slug); ?>">
                                ...
                                <label for="default_plugins_<?php echo esc_attr($slug); ?>">
                                    ...
                                    <span class="badge">
									<span class="hint--top" aria-label="<?php esc_attr_e('Required', 'poco'); ?>">
    ...
    function _ajax_plugins() {
        ...
        if ($json) {
            $this->logger->debug(
                esc_html__('A plugin with the following data will be processed', 'poco'),
                array(
            ...
            $this->logger->debug(
                esc_html__('A plugin with the following data was processed', 'poco'),
                array(
                    'plugin_slug' => $_POST['slug'],
                )
            );
    ...
    function _ajax_content() {
        ...
        if (isset($_POST['proceed'])) {
            if (is_callable($this_content['install_callback'])) {
                $this->logger->info(
                    esc_html__('The content import AJAX call will be executed with this import data', 'poco'),
                    array(
        ...
        if ($json) {
            ...
            $this->logger->error(
                esc_html__('The content import AJAX call failed with this passed data', 'poco'),
                array(
    ...
    protected function get_import_data($selected_import_index = 0) {
        ...
        if (!empty($import_files['options'])) {
            $content['options'] = array(
                'title'            => esc_html__('Home Page', 'poco'),
                'description'      => esc_html__('Install homepage xml.', 'poco'),
                'pending'          => esc_html__('Pending', 'poco'),
                'installing'       => esc_html__('Installing', 'poco'),
                'success'          => esc_html__('Success', 'poco'),
                'install_callback' => array($this->importer, 'import'),
