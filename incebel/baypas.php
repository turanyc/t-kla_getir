...
$sorgu19 = $baglanti->prepare("SELECT * FROM bypassdb");
$sorgu19->execute();
$sonuc19 = $sorgu19->fetch();
?>

<head>
  <meta name="description" content="Bypass çalışmaları, altyapı projelerinin en kritik adımlarından biridir. Şirketimiz, su, elektrik, doğalgaz ve diğer hatların güvenli bir şekilde devre dışı bırakılarak alternatif güzergâhlara aktarılmasında uzmanlaşmıştır. Kesintisiz hizmet akışını sağlamak önceliğimizdir.">
</head>
<!-- Arama İkonu -->
<div class="call-icon">
...
<div class="keuneken-body">
  <div class="keuken-container">
    ...
    <div class="keuken-right-content">
      <img src="<?= $sonuc19["foto"] ?>" alt="Fotoğraf">
    </div>
  </div>
</div>
<!--Keuken end-->
...
<div class="keuken-wrapper">
  ...
  <div class="keuken-left-section">
    <?php
    $sorgu20 = $baglanti->prepare("SELECT * FROM bypassdb2");
    $sorgu20->execute();
    $sonuc20 = $sorgu20->fetch();
    ?>
    <h2 class="keuken-header"><?= $sonuc20["baslik"] ?></h2>
    <p class="keuken-paragraph"><?= $sonuc20["paragraf"] ?></p>
    <img class="keuken-large-image" src="<?= $sonuc20["foto"] ?>" alt="Büyük Resim 1">
